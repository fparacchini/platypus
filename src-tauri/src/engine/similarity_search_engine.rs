...
pub const DEFAULT_RAG_TOP_K: usize = 20; // Default top K chunks for RAG retrieval
pub const MAX_NB_CONNECTION: usize = 50; // Keep high for HNSW graph quality
...
impl SimilaritySearch {
    ...
    /// Search for top-k similar items, filtered to only include specified IDs
    /// This is used for project-scoped searches
    pub async fn top_k_filtered(
        ...
        let search_k = std::cmp::min(top_k * 3, MAX_NB_CONNECTION);
        
        info!(
            "Performing filtered similarity search: Query={}, searching {} items, filtering to {} allowed IDs",
        ...
        let filtered: Vec<(usize, f32)> = all_results
            .into_iter()
            .filter(|(id, _)| allowed_set.contains(&(*id as i64)))
            .take(top_k)
            .collect();

        info!(
            "Filtered similarity search completed. Retrieved {} relevant chunks",
            filtered.len()
        );
