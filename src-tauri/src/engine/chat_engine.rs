...
pub async fn send_prompt_to_llm(
    ...
    if is_first_message {
        ...
        if let Some(pid) = project_id {
            ...
            match search_project_vectors_live(&app_handle, pid, &user_prompt, rag_top_k, &setting_openai.setting_value, embed_api_base_opt, embed_model_opt).await {
            ...
            for line in text.lines() {
                ...
                match json_data["type"].as_str() {
                    Some("message_start") => {
                        ...
                        if let Some(model) = json_data["message"]["model"].as_str() {
                    ...
                    Some("message_stop") => {
                        info!("Response complete - Input: {} tokens, Output: {} tokens, Total: {} tokens",
